<div class="main-card mb-3 card">
    ...
    <div class="card-header text-right">
        <form action="{{route('pacientes-export.update', $paciente->id)}}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="card mb-2">
                        <div class="card-body">
                            <h5 class="card-title">Data:
                                {{ $prontuario->data_monitoramento ? $prontuario->data_monitoramento->format('d/m/Y') : 'Data não definida' }}
                                - Hora: {{ $prontuario->horario_monitoramento }}</h5>
                            <p class="card-text">
                            ...
                            <p class="card-text">
                                <strong>Frequência Respiratória:
                                </strong>{{ $prontuario->frequencia_respiratoria_atual }}
                            </p>
                            <p class="card-text">
                            ...
                            <p class="card-text">
                                <strong>Medicamento Prescrito?
                                </strong>{{ human_boolean($prontuario->equipe_medica) }}
                            </p>
                            ...
                            <p class="card-text">
                                <strong>Sentiu Melhoras com Escaldapés:
                                </strong>{{ ucfirst($prontuario->melhora_sintoma_escaldapes) }}
                            </p>
                            <p class="card-text">
                                <strong>Inalação/Vaporização: </strong>{{ ucfirst($prontuario->fes_inalacao) }}
                            </p>
                            <p class="card-text">
                                <strong>Sentiu Melhoras com Inalação/Vaporização:
                                </strong>{{ ucfirst($prontuario->melhoria_sintomas_inalacao) }}
                            </p>
                        </div>
                    </div>
                </div>
